...
namespace WindowsFormsApp2
{
    public partial class Form11 : Form
    {
        ...
        {
            string query = "SELECT * FROM user;"; // SQL-запрос для выборки всех данных из таблицы program
        ...
        private void DeleteRowsFromDatabase(List<int> ids)
        {
            try
            {
                ...
                {
                    string query = $"DELETE FROM user WHERE id = {id};"; // Предположим, что "program" - это имя вашей таблицы
                    MySqlCommand cmd = new MySqlCommand(query, connection);
                    cmd.ExecuteNonQuery();
                }
            }
