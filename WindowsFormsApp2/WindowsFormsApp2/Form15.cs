...
namespace WindowsFormsApp2
{
    public partial class Form15 : Form
    {
        ...
        public Form15(int background, int plauplay, string username)
        {
            ...
            toolTip1.SetToolTip(guna2TextBox1, "получаная общая сумму");
        ...
        private void SaveToDatabase(string Username)
        {
            ...
            // Проверяем, заполнено ли поле для сохранения данных
            if (!string.IsNullOrWhiteSpace(guna2TextBox1.Text))
            {
                using (ExcelPackage package = new ExcelPackage(newFile))
                {
                    string sheetName = "общую сумму все время"; // Имя листа

                    // Получаем лист для данных пользователя, если он уже существует
                    ExcelWorksheet worksheet = package.Workbook.Worksheets.FirstOrDefault(sheet => sheet.Name == sheetName);

                    // Если лист не найден, создаем новый с указанным именем
                    if (worksheet == null)
                    {
                    ...
                    int nextRow = worksheet.Dimension?.Rows + 1 ?? 1; // Определяем следующую строку для записи
                    worksheet.Cells[nextRow, 2].Value = guna2TextBox1.Text; // Записываем данные во второй столбец следующей строки
        ...
        private void ClearWorksheet(ExcelWorksheet worksheet)
        {
